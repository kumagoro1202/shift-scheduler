...
def calculate_skill_score(employee: Dict[str, Any], time_slot: Dict[str, Any]) -> int:
    ...
    if area_type == 'リハ室':
        return employee.get('skill_reha_room', employee.get('skill_score', 0))
    elif area_type == '受付':
        if time_period == '午前':
    ...
    return employee.get('skill_flexibility', employee.get('skill_score', 0))
...
def can_assign_to_area(employee: Dict[str, Any], time_slot: Dict[str, Any]) -> bool:
    ...
    if area_type == 'リハ室':
        ...
        if emp_type not in ['TYPE_A', 'TYPE_C', 'TYPE_D']:
            return False
        # リハ室スキルが0以下は不可
        if employee.get('skill_reha_room', 0) <= 0:
            return False
        ...
        for date in dates:
            ...
            for ts in time_slots:
                ...
                if len(available_employees) < req_min:
                    print(f"❌ {date} {ts['name']}: 勤務可能{len(available_employees)}名 < 最小必要{req_min}名")
                    return None
