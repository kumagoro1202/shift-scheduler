...
本システムでは、職員の雇用形態と勤務形態に応じて、勤務パターンの割り当て方法が異なります。

**パターン固定について**:
- **🔄 変動**: 正職員（通常）は日によって早番・中番・遅番を変動的に割り当て
- **📌 固定**: パート職員・時短勤務職員は登録された勤務形態で固定

**正職員の勤務パターン変動割り当て**:

正職員（is_pattern_fixed=False）は、シフト生成時に曜日と状況に応じて以下のパターンから自動的に選択されます：

| 曜日 | パターンA/◯/1 | パターンB/通常/2 | パターンC/△/3 | 説明 |
|------|--------------|----------------|--------------|------|
| 月火金 | A (08:30-17:30) | B (08:45-18:45) | C (09:15-19:15) | 開院準備・標準・閉院作業 |
| 水曜 | ◯ (08:30-16:30) | 通常 (08:45-17:45) | △ (09:15-18:15) | 午後短縮診療日 |
| 木土 | 1 (08:30-12:30) | 2 (08:45-12:45) | 3 (09:00-13:00) | 午前診療のみ |

**パターン選択の特徴**:
- 同一職員が特定のパターンに偏らないよう自動調整
- 開院準備（早出）や閉院作業（遅出）の担当が固定化されない
- 最適化アルゴリズムがスキルバランスと勤務パターンを考慮
- パート・時短職員は固定パターンで安定した働き方が可能

**例**: 田中さん（正職員・通常）の場合
- 月曜日: パターンB (08:45-18:45)
- 火曜日: パターンA (08:30-17:30)
- 水曜日: 通常 (08:45-17:45)
- 金曜日: パターンC (09:15-19:15)
- 土曜日: パターン2 (08:45-12:45)

このように、日によって異なるパターンが割り当てられ、負担が公平に分散されます。

**パート職員の固定パターン**:

パート職員や時短勤務職員（is_pattern_fixed=True）は、職員管理画面で登録した勤務形態で固定されます：
...
**⚖️ バランスモード**は、以下の要素を総合的に考慮してシフトを生成します：
...
1. 勤務日数が最も少ない職員を抽出
   ↓
2. その中からスキルが目標に近い職員を選択
   ↓
3. 正職員には偏りのない勤務パターンを割り当て
   ↓
4. 全体のバランスを保ちながら公平に配分
```
