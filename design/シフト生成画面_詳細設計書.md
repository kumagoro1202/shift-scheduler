...
### 1.4 設計思想
- **最適化エンジン**: 職員タイプ制約、スキルバランス、勤務回数均等化、勤務パターン選択を考慮
...
- **勤務パターンの選択**:
  - 正職員（通常）: シフト生成時に早番・中番・遅番から最適なパターンを選択
  - パート・時短職員: 登録された勤務パターンで固定
...
| employment_pattern_id | str | - | 勤務パターンID（正職員は日ごとに変動、パート・時短は固定） |
...
| employment_pattern_id | str | - | FK | 勤務パターンID（正職員は日ごとに変動、パート・時短は固定） |
...
**月単位で選択の場合**:
- `get_month_range(year, month)` で21日から翌月20日までの期間を取得
...
sequenceDiagram
    ...
    alt 生成成功
        ...
        loop 各シフト
            UI->>DB: create_shift(date, slot_id, emp_id, pattern_id)
...
while current <= end_date:
    ...
    # パートタイムルールの検証
    # 正職員の勤務パターン選択（早番・中番・遅番）
    # パート・時短職員は登録されたパターンを使用
...
**正職員（通常）のパターン選択**:
```python
def _select_employment_pattern(employee, date, time_slot, daily_assignments):
    """正職員の勤務パターンを選択する。
    
    正職員（is_pattern_fixed=False）の場合、日によって
    早番・中番・遅番から最適なパターンを選択する。
    """
    # パターン固定の職員（パート・時短）
    if employee.is_pattern_fixed:
        return employee.employment_pattern_id
    
    # 正職員（通常）の場合、曜日に応じたパターンから選択
    weekday = date.weekday()
    
    if weekday in [0, 1, 4]:  # 月火金
        candidates = ['weekday_full_A', 'weekday_full_B', 'weekday_full_C']
    elif weekday == 2:  # 水曜
        candidates = ['wednesday_early', 'wednesday_normal', 'wednesday_late']
    elif weekday in [3, 5]:  # 木土
        candidates = ['thu_sat_1', 'thu_sat_2', 'thu_sat_3']
    else:
        return None  # 日曜日は休診
    
    # 同一職員が連続して同じパターンにならないよう配慮
    # 前日のパターンを確認し、異なるパターンを優先
    previous_pattern = daily_assignments.get((employee.id, date - timedelta(days=1)))
    
    if previous_pattern and previous_pattern in candidates:
        # 前日と異なるパターンを優先
        other_patterns = [p for p in candidates if p != previous_pattern]
        if other_patterns:
            return other_patterns[0]  # 簡略化した選択
...
**パターン選択の原則**:
- パート・時短職員: 登録された`employment_pattern_id`を使用
- 正職員（通常）: 曜日に応じたA/B/C、◯/通常/△、1/2/3から選択
- 同一職員が特定のパターンに偏らないよう、使用回数を追跡
- 各時間帯で必要な人員配置を優先しつつ、パターンの多様性を保つ

**実装詳細**:
- `optimizer.py`の`_get_available_patterns_for_day()`で曜日判定
- `_select_best_pattern()`で使用回数が最少のパターンを優先選択
- `pattern_usage_count: Dict[tuple[int, str], int]`で(employee_id, pattern_id)ごとに使用回数を管理
...
| create_shift | database.py | シフト作成 | date: str, time_slot_id: str, employee_id: int, employment_pattern_id: str | Optional[int] |
...
for payload in shift_payloads:
    shift_id = create_shift(date, time_slot_id, employee_id, employment_pattern_id)
