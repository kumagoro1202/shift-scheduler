...
| employee_type | str | ○ | TYPE_A | CHECK | 職員タイプ（TYPE_A/B/C/D/E/F） |
| employment_type | str | ○ | 正職員 | CHECK | 雇用形態（正職員/パート） |
| employment_pattern_id | str | - | None | FK | 勤務パターンID（パート・時短職員用） |
| is_pattern_fixed | bool | ○ | False | - | 勤務パターン固定フラグ（True: 固定, False: 変動） |
| skill_reha | int | ○ | 50 | 0-100 | リハ室スキル |
| skill_reception_am | int | ○ | 50 | 0-100 | 受付午前スキル（医事業務能力を優先） |
| skill_reception_pm | int | ○ | 50 | 0-100 | 受付午後スキル（医事業務能力を優先） |
...
| TYPE_B | 受付専任 | 受付AM、受付PM、総合（リハ室は0固定） |
| TYPE_C | リハ室専任（正職員） | リハ室、総合（受付系は0固定） |
| TYPE_D | リハ室専任（パート） | リハ室、総合（受付系は0固定） |
| TYPE_E | 受付専任（パート） | 受付AM、受付PM、総合（リハ室は0固定） |
| TYPE_F | 受付専任（時短） | 受付AM、受付PM、総合（リハ室は0固定） |

### 2.2 EmploymentPattern（勤務パターン）エンティティ

**重要**: 勤務パターンの割り当て方針
- **正職員**: 日によって早番・中番・遅番を変動的に割り当て（シフト生成時に決定）
- **パート職員・時短勤務職員**: 登録された勤務パターンで固定
...
| break_hours | float | ○ | 休憩時間（時間）。正職員は基本2時間連続、忙しい日は分割可 |
...
| 時短勤務職員 | 時短パターンID | True | 登録されたパターンで固定 |
| パート職員 | パートパターンID | True | 登録されたパターンで固定 |
...
|--------|------|--------|--------|-----------|
| 1 | 職員タイプ | selectbox | 全て、リハ室・受付両方可能、受付専任、リハ室専任(正職員)、リハ室専任(パート)、受付専任(パート)、受付専任(時短) | 全て |
...
**配置**: 11カラムレイアウト（比率: 2:2:2:2:1:1:1:1:1:1:1.5）

**注**: 正職員（通常）は勤務パターンが「変動」と表示され、パート・時短職員は具体的なパターン名が表示されます。
...
| 4 | 勤務パターン | パターン名と時間（正職員通常は「変動」） | text / selectbox（編集時） |
| 5 | パターン固定 | 固定/変動 | text（アイコン表示: 📌固定/🔄変動） |
| 6 | リハ室 | リハ室スキル | text / number_input（編集時） |
| 7 | 受付AM | 受付午前スキル | text / number_input（編集時） |
| 8 | 受付PM | 受付午後スキル | text / number_input（編集時） |
| 9 | 総合 | 総合対応力 | text / number_input（編集時） |
| 10 | 合計 | 4項目の合計値 | text（太字） |
| 11 | 操作 | 編集・削除ボタン | button |
...
| 基本情報 | 職員タイプ | selectbox | ○ | TYPE_A/B/C/D/E/F | TYPE_A | 2カラムの右、日本語ラベル表示 |
| 基本情報 | 勤務区分 | radio | △ | full_time/short_time | full_time | 正職員の場合のみ表示 |
| 基本情報 | 勤務パターン | selectbox | △ | カテゴリに応じた選択肢 | 先頭のパターン | パート・時短職員のみ必須。正職員は参考用 |
| 基本情報 | パターン固定 | checkbox | - | True/False | 自動設定 | パート・時短の場合はTrue、正職員（通常）はFalse |
| スキル | リハ室スキル | number_input | ○ | 0-100 | 0 | TYPE_B/E/Fは無効化 |
| スキル | 受付午前スキル | number_input | ○ | 0-100 | 0 | TYPE_C/Dは無効化。医事業務能力を優先 |
| スキル | 受付午後スキル | number_input | ○ | 0-100 | 0 | TYPE_C/Dは無効化。医事業務能力を優先 |
...
if employment_type == "正職員":
    ...
    if pattern_category == "short_time":
        is_pattern_fixed = True  # 時短勤務は固定
    else:
        is_pattern_fixed = False  # 通常正職員は変動
else:
    pattern_category = "part_time"
    is_pattern_fixed = True  # パートは固定
...
| 勤務パターンについて | 勤務パターンの概要説明、正職員の変動割り当てとパート・時短の固定割り当ての違い |
...
def get_skill_constraints(employee_type: str) -> dict:
    ...
    elif employee_type in ["TYPE_B", "TYPE_E", "TYPE_F"]:
        return {
...
for emp in employees:
    ...
    employee_data.append({
        ...
        'is_pattern_fixed': emp.is_pattern_fixed,
...
| TYPE_B | 受付専任 | 📋 TYPE_B (受付専任) |
| TYPE_C | リハ室専任(正) | 💪 TYPE_C (リハ室専任・正職員) |
| TYPE_D | リハ室専任(パ) | 🏃 TYPE_D (リハ室専任・パート) |
| TYPE_E | 受付専任(パ) | 📝 TYPE_E (受付専任・パート) |
| TYPE_F | 受付専任(時短) | ⏰ TYPE_F (受付専任・時短) |
...
| create_employee | database.py | 職員作成 | name, employee_type, employment_type, employment_pattern_id, is_pattern_fixed, skill_* | int (employee_id) |
...
#### CHECK制約
- `employee_type`: IN ('TYPE_A', 'TYPE_B', 'TYPE_C', 'TYPE_D', 'TYPE_E', 'TYPE_F')
...
| 正常登録（TYPE_E） | リハ室スキル無効 | 登録成功、リハ室スキル=0 |
| 正常登録（TYPE_F） | リハ室スキル無効 | 登録成功、リハ室スキル=0 |
| 正常登録（正職員・通常） | フルタイム選択 | 登録成功、is_pattern_fixed=False |
| 正常登録（正職員・時短） | 時短勤務選択 | 登録成功、is_pattern_fixed=True |
| 正常登録（パート） | パート選択 | 登録成功、is_pattern_fixed=True |
...
| テストケース | 操作 | 期待結果 |
|------------|------|---------|
| スキル制約（TYPE_B/E/F） | TYPE_B/E/F選択時にリハ室スキル入力 | リハ室スキル入力欄が無効化 |
| スキル制約（TYPE_C/D） | TYPE_C/D選択時に受付スキル入力 | 受付スキル入力欄が無効化 |
